...
//  import system.dispatcher // not recommended in practice for mapAsync
  ...
  object PagerService {
    ...
    def processEvent(pagerEvent: PagerEvent): Future[String] = Future {
      ...
      val engineerEmail = emails(engineers(engineerIndex.toInt))
  ...
  val pagedEngineerEmails = infraEvents
    .mapAsync(parallelism = 1)(event => PagerService.processEvent(event)) //guarantees the relative order of elements
