...
object IntegratingWithActors extends App {
  ...
  val actorBasedFlow = Flow[Int].ask[Int](parallelism = 4)(simpleActor) // flow from Int to Int
...
//  numberSource.ask[Int](parallelism = 4)(simpleActor).to(Sink.ignore).run()
  ...
  val materializedActorRef = actorPoweredSource
    .to(Sink.foreach[Int](number => println(s"Actor powered flow got number: $number"))).run()
  ...
  // Sink.actorRef() not recommended, not able to backpressure
}
