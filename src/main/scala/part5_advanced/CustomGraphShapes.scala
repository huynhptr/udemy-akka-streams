...
object CustomGraphShapes extends App {
  ...
  val balance2x3Imp = GraphDSL.create() { implicit builder =>
  ...
  val balance2x3Graph = RunnableGraph.fromGraph(
    GraphDSL.create() { implicit builder =>
      ...
      def createSinks(index: Int) = Sink.fold(0)((count: Int, element: Int) => {
        println(s"[Sink $index] Received $element, current count is $count")
        count + 1
      })

      val sink1 = builder.add(createSinks(1))
      val sink2 = builder.add(createSinks(2))
      val sink3 = builder.add(createSinks(3))

      val balance2x3 = builder.add(balance2x3Imp)
...
//  balance2x3Graph.run()
  ...
  /*
  Exercise: generalize the balance component, make it MxN and also generalize its type
   */
  // balance MxN shape (my solution)
//  case class UniformBalanceMxN[T](ins: List[Inlet[T]], outs: List[Outlet[T]]) extends Shape {
//    // Inlet[T], Outlet[T]
//    override def inlets: immutable.Seq[Inlet[_]] = ins
//
//    override def outlets: immutable.Seq[Outlet[_]] = outs
//
//    override def deepCopy(): Shape = UniformBalanceMxN(
//      ins.map(in => in.carbonCopy()),
//      outs.map(out => out.carbonCopy())
//    )
//  }
